...
function CarWashCanvas({ background, lights, pedControls }) {
  const style = { backgroundImage: `url(${background})` };
  return (
    <div className="carwash" style={style}>
    ...
    <Row className="carwashpage">
      <Col xs={12} lg={8}>
        ...
        <div className="mt-3">
          ...
          <CarWashCanvas background = {background} lights = {lights} pedControls = {{ btnPedN: btnPed1, btnPedS: btnPed2, onNorth: togglePedN, onSouth: togglePedS }} /> 
        </div>
      ...
      <Col lg={4}>
        <CarWashParamsSidebar names={names} idx={idx} onPrev={prev} onNext={next} onJump={jump}/>
      </Col>
    </Row>
  );
}
