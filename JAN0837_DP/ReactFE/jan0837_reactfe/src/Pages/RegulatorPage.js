...
function RegulatorParamsSidebar({names, idx, onPrev, onNext, onJump,}) 
{
  ...
  const btnStart = toBool(d?.btnStart_regulator);
  const R = Number(d?.R ?? 0);
  const C = Number(d?.C ?? 0);
  const U = Number(d?.U ?? 0);
  const I = Number(d?.I ?? 0);

  const setStartAsync = () => saveSection({ btnStart_regulator: !btnStart });
...
function RegulatorPage({ setAside }) {
  ...
  const { section: d, saveSection, data } = useSectionData('RegulatorData');

    useEffect(() => {
    // RegulatorData doesn't need traffic light init
  }, []);
  ...
  */}
  ...
  const isNight = false; // Regulator doesn't use day/night
